...
function AdminCarSubsidyReviewPurchaseTaxDetailPage() {
  const { search } = useLocation();
  const [detailDate, setDetail] = useState({});
  const localStore = useLocalStore(() => ({}));

  // const store = useStore('modelAdminCarSubsidyReviewPurchaseTaxDetail'); // 注意store里面的数据不要使用解构赋值
  // console.log(store);
...
export default observer(
  CSSModules(AdminCarSubsidyReviewPurchaseTaxDetailPage, styles, {
    allowMultiple: true,
  }),
);
