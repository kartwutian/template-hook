...
function UserCarSubsidyMngPurchaseTaxListPage() {
  ...
  const handleToEdit = (id) => {
    history.push({
      pathname: '/car_subsidy_mng/purchase_tax/update',
      search: `?id=${id}`,
    });
  };
  ...
  const handleToDetail = (id) => {
    console.log('详情', id);
    history.push(`/car_subsidy_mng/purchase_tax/detail?id=${id}`);
  };
  ...
  const handleAddValueTax = () => {
    history.push({
      pathname: '/car_subsidy_mng/purchase_tax/create',
    });
  };
...
export default observer(
  CSSModules(UserCarSubsidyMngPurchaseTaxListPage, styles, {
    allowMultiple: true,
  }),
);
